...
void Renderer::init()
{
	...
	glfwSetWindowSizeCallback(display.getWindow(), Renderer::onWindowResized);
...
{
	auto vertShaderCode = readFile("C:\\Users\\Dynamitos5\\Documents\\Schule\\POS\\Vulkan Test\\external\\shaders\\triangleVertex.spv");
	auto fragShaderCode = readFile("C:\\Users\\Dynamitos5\\Documents\\Schule\\POS\\Vulkan Test\\external\\shaders\\triangleFragment.spv");
...
void Renderer::createCommandPool()
{
	...
	VK_CHECK(vkCreateCommandPool(device, &poolInfo, nullptr, &cmdTempPool));
}

void Renderer::createTextureImage()
{
	int texWidth, texHeight, texChannels;
	stbi_uc* pixels = stbi_load("C:\\Users\\Dynamitos5\\Documents\\Schule\\POS\\Vulkan Test\\external\\textures\\ic_launcher.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
void Renderer::onWindowResized(GLFWwindow* window, int width, int height)
{
	...
	Renderer* display = reinterpret_cast<Renderer*>(glfwGetWindowUserPointer(window));
...
void Renderer::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VDeleter<VkBuffer>& buffer, VDeleter<VkDeviceMemory>& bufferMemory)
{
...
void Renderer::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VDeleter<VkImage>& image, VDeleter<VkDeviceMemory>& imageMemory)
{
...
VkCommandBuffer Renderer::beginSingleTimeCommands()
{
	...
	allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
	allocInfo.commandPool = cmdTempPool;
...
void Renderer::endSingleTimeCommands(VkCommandBuffer commandBuffer)
{
	...
	vkFreeCommandBuffers(device, cmdTempPool, 1, &commandBuffer);
}
